...
public class SampleMecanumDriveCancelable extends MecanumDrive {
    public static PIDCoefficients TRANSLATIONAL_PID = new PIDCoefficients(8.5, 0, 0); //check with spline
    public static PIDCoefficients HEADING_PID = new PIDCoefficients(8, 0, 0);

    public static double LATERAL_MULTIPLIER = 1.4; //1.4 mess with later imo
...
//        imu.initialize(parameters);
    ...
    public SampleMecanumDriveCancelable(HardwareMap hardwareMap, AllianceColor a) {

        super(kV, kA, kStatic, TRACK_WIDTH, TRACK_WIDTH, LATERAL_MULTIPLIER);

        if (a==AllianceColor.BLUE){
            ACCEL_CONSTRAINT = getAccelerationConstraint(45);
        }
        follower = new HolonomicPIDVAFollower(TRANSLATIONAL_PID, TRANSLATIONAL_PID, HEADING_PID,
                new Pose2d(0.5, 0.5, Math.toRadians(5.0)), 0.5);
...
//        imu.initialize(parameters);
    ...
    public double getCurrent() {
        return ((leftFront.getCurrent(CurrentUnit.AMPS)+rightFront.getCurrent(CurrentUnit.AMPS)+leftRear.getCurrent(CurrentUnit.AMPS)+ rightRear.getCurrent(CurrentUnit.AMPS))/4.0);
    }
}